...
def translation_yaml(**overrides: object) -> str:
...
def test_translation_model_loader_uses_protocol_and_reads_relative_key(
    tmp_path: Path,
) -> None:
    key_path = tmp_path / "secrets/provider.key"
    key_path.parent.mkdir()
    key_path.write_text("  secret-value\n", encoding="utf-8")
    config_path = write_config(tmp_path / "workflow.yaml", translation_yaml())

    settings = load_translation_model_settings(config_path)

    assert settings == TranslationModelSettings(
        config=ModelConfig(
...
def test_translation_model_loader_rejects_invalid_config(
    ...
    config_path = write_config(
        tmp_path / "workflow.yaml", translation_yaml(**overrides)
    )

    with pytest.raises(ValueError, match=match):
        load_translation_model_settings(config_path)


def test_translation_model_loader_rejects_empty_key_and_unknown_field(
    tmp_path: Path,
) -> None:
    key_path = tmp_path / "secrets/provider.key"
    key_path.parent.mkdir()
    key_path.write_text(" \n", encoding="utf-8")
    empty_key_config = write_config(tmp_path / "empty.yaml", translation_yaml())

    with pytest.raises(ValueError, match="key file must not be empty"):
        load_translation_model_settings(empty_key_config)

    unknown_config = write_config(
        tmp_path / "unknown.yaml",
        translation_yaml(extra="unsupported"),
    )
    with pytest.raises(ValueError, match="unknown fields: extra"):
        load_translation_model_settings(unknown_config)
